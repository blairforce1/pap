...
| `scripts/tidy.sh`                 | Run by `mise run tidy` after a change merges. Fetches with `--prune`, then for each local branch whose upstream is gone force-removes its worktree and deletes the branch with `-D`: a squash merge leaves the tip off `main`, so `--merged` never sees it. Never touches the main checkout, the current branch, a branch with a live upstream or none, or a worktree with uncommitted tracked changes, which it reports and skips. Prints what it removed and what it left. `pap doctor` counts the branches it would take.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
