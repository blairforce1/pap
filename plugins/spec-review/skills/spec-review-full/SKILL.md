...
2. **Run each perspective** by loading and following `${CLAUDE_PLUGIN_ROOT}/skills/spec-review-<perspective>/SKILL.md` for each selected perspective. Where the environment supports it, run perspectives as parallel subagents, each returning its findings table; otherwise run them sequentially. Give each perspective the PAP context you gathered in step 1 (class, the invariants touched and their text, the spec's Areas of concern) so it can fill its Invariant column. Apply each perspective fully; do not summarise a perspective away because another already raised something similar.
