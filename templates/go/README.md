...
| `.config/lefthook/go.yml` | pre-commit: `gofmt -l` on staged `.go` files, failing on any file it lists or on a syntax error (`gofmt -l` itself exits 0 when it lists files). pre-push: `go vet ./...` and `golangci-lint run` when the push changes a `.go` file, and `govulncheck ./...` when it changes a `.go` file, `go.mod` or `go.sum`, since a version bump alone can bring a vulnerable dependency in. |
...
- An unchecked error and a `go vet` violation (`%d` with a string): the
  ...
  `go-vet` (0.05 s) and by `golangci-lint` (0.2 s), which reports `errcheck`,
  `govet` and `unused`.
...
- **`go vet` and golangci-lint's `govet`**: both run on pre-push, and they
  overlap. `go vet` runs the toolchain's default analysers with no flags;
