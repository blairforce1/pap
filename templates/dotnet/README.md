...
| `.config/lefthook/dotnet.yml` | A pre-commit job, pulled in by the base `lefthook.yml`, running `dotnet format --verify-no-changes --include` on staged `.cs` files. Measured: 3.4 to 4.5 s on a one-project console solution and 3.7 to 4.9 s with a class library added, over five runs each, so it stays in pre-commit, close to the line. Nearly all of that is loading the workspace, so it grows with the solution, not with the files staged; move it to pre-push when it passes five seconds. |
...
- A file `dotnet new` scaffolds arrives with a UTF-8 byte-order mark, which
  ...
  files only; the base `mise run fmt` strips it from every text file,
  `.csproj` and `.slnx` included, before `dotnet format` runs.
...
mise run fmt                        # after `dotnet new`: strips the BOM it writes, then formats
