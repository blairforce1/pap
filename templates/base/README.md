...
| `.gitignore` | OS, editor, .NET, Go and node artefacts, in commented sections. Go binaries have no fixed extension, so the Go section ignores test binaries, coverage profiles and pprof output, and relies on `bin/` for `go build -o bin/`. Editor settings directories are ignored whole. |
