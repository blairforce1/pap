...
| 5.1 | Agents do not bypass hooks. An agent session never sets `LEFTHOOK` or `LEFTHOOK_EXCLUDE`, passes `--no-verify` or `commit -n`, or sets `core.hooksPath` in a repository that adopts PAP; the hatches are for humans. | [0005](decisions/0005-agent-sessions-do-not-bypass-hooks.md) | Claude Code `PreToolUse` hook [`plugins/pap/hooks/guard-route.sh`](plugins/pap/hooks/guard-route.sh) via [`hooks.json`](plugins/pap/hooks/hooks.json), from pap plugin 0.3.0. It does not follow `bash -c`, `eval` or scripts. No server-side backstop until the CI scanning layer exists. |
